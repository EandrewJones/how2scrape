...
`Robots.txt` are a website's 'rules of conduct' for web crawlers trawling their server. They are text documents telling your bot *if*, *how*, and *which* part of the website is open for scraping. Most websites you are interested in crawling will have one, and many sites prohibit crawling of any kind. When trying to extract information from websites, you ought to know what the `robots.txt` says and how to abide by it to avoid legal issues down the line (I don't know about you, but I can't afford a legal retainer fee on top of UMD's mandatory fees).
...
If you see a forward-slash next to disallow, this means everything down to the root node (/) of the server is off-limits. Avoid crawling such sites.
